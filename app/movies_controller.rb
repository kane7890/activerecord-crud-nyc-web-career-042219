...
def can_find_the_first_item_from_the_database_using_id
  moviefirstid=Movie.limit(1).first
  # binding.pry
  moviefirstid
end
...
def can_find_by_multiple_attributes
  ...
  release_date = 2000
  director = "Me"
  movieattr=Movie.where("title = ? and release_date = ? and director = ?", title, release_date, director).first
    # if self.title == "Title" || self.release_date == 2000 || self.director == "Me"
    #   self
    # else
    #   return(nil)
    # end
...
def can_find_using_where_clause_and_be_sorted
  ...
  movieattr=Movie.where("release_date > 2002").order(release_date: :desc)
  # binding.pry
  movieattr
...
def can_update_using_update_method
  ...
  tmpid=Movie.create(title: "Wat?").id
  Movie.update(tmpid, title: "Wat, huh?")
